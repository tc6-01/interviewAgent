...
默认构建产物写入 `../internal/webui/dist/`，供 Go `go:embed` 同源交付：

```bash
VITE_API_MODE=real npm run build
```

GitHub Pages 只允许发布 `VITE_API_MODE=mock` 的静态预览。需要 Pages 产物时可执行 `VITE_API_MODE=mock npm run build -- --outDir dist`；该预览不连接正式匿名后端。
