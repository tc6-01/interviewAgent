...
func (q *QuickQuizSkill) Description() string  { return "快速测验——输入主题即可练习面试题，即时评分反馈" }
...
func (q *QuickQuizSkill) retrieveQuestions(ctx context.Context, topic string, count int, userID string) ([]quizQuestion, error) {
	if userID == "" {
		userID = "default_user"
	}
