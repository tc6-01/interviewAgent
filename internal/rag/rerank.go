...
// Reranker 基于 LLM 的重排序器
type Reranker struct {
	chatModel model.ChatModel
	topK      int
}

// NewReranker 创建 LLM 重排序器
func NewReranker(chatModel model.ChatModel, topK int) *Reranker {
	if topK <= 0 {
		topK = 5
	}
	return &Reranker{
...
func (r *Reranker) Rerank(ctx context.Context, query string, docs []*schema.Document) ([]*schema.Document, error) {
	...
	// 这是「全量重排、绝不过滤」的代码层兜底：rerank 只改善排序、不缩小召回集合，
	// 避免「过滤式 rerank」误删相关题拉低 Recall（实测整体 0.7567→0.6533）。
	...
	// 截断到 topK（先全量重排，再截断）
	if len(result) > r.topK {
		result = result[:r.topK]
	}
	return result, nil
}

// ScoreBasedReranker 基于分数的简单重排序器（不依赖 LLM）
...
func (r *CrossEncoderReranker) Rerank(ctx context.Context, query string, docs []*schema.Document) ([]*schema.Document, error) {
	...
	if err != nil {
		return r.truncate(docs), nil // 网络失败降级
	}
	defer resp.Body.Close()
	if resp.StatusCode != http.StatusOK {
		return r.truncate(docs), nil // 403 等降级
	}
...
func (r *CrossEncoderReranker) truncate(docs []*schema.Document) []*schema.Document {
	if len(docs) > r.topK {
		return docs[:r.topK]
	}
	return docs
...
// NoneReranker 不重排，仅截断到 topK（用于离线评估做对照基线）。
...
// Rerank 直接返回原始顺序、截断 topK。
func (r *NoneReranker) Rerank(_ context.Context, _ string, docs []*schema.Document) ([]*schema.Document, error) {
	if len(docs) > r.topK {
